...
module.exports = function () {
  this.run_sass_autoprefixer = function (src, destination, autoprefixerConfig, cssnanoConfig) {
    ...
    this.pump([
      ...
      this.gulpif(this.config.app.input.compiler.styles.sass === true, this.sass().on('error', function (err) {
        ...
        this.emit('end')
        return false
      }), this.gulpif(this.config.app.input.compiler.styles.less === true, this.stylus().on('error', function (err) {
        this.gutil.log(this.gutil.colors.red('[Error]'), err.toString())
        this.emit('end')
        return false
      })))),
  ...
  this.run_uglify = function (src, destination) {
    this.pump([
      // this.sourcemaps.init(),
      this.gulpif(this.config.app.input.compiler.scripts.babel === true, this.gulp.src([
      ...
      this.strip(),
      this.gulp.dest(destination),
      this.gulpif(this.config.app.output.uglify === true, this.uglify().on('error', function (err) {
        this.gutil.log(this.gutil.colors.red('[Error]'), err.toString())
        this.emit('end')
        return false
      }), this.gulp.dest(destination)),
  ...
  function push (globs, dest, styles) {
    ...
    if (this.fs.existsSync(globs[0].substring(0, globs[0].lastIndexOf('/')))) {
      ...
      this.gutil.log(this.gutil.colors.red('[Error]'), "Dist file doesn't exists, please verify your paths & files -> " +
        globs[0])
      return null
    }
  }
  this.uploadStyles = function () {
    ...
    var globs = [rootDist + this.config.app.watch.styles]
    push(globs, base, true)
  }
  this.uploadScripts = function () {
    var base = this.config.ftp.base
    var rootDist = this.appRoot.path + this.config.app.paths.dist
    var globs = [rootDist + this.config.app.watch.scripts]
